...
from torch import grid_sampler_3d, nn
...
from gs3d.scene import GaussianModel
from gs3d.gaussian_renderer import render
from gs3d.scene.cameras import Camera, KCamera
...
class DeformableGaussian3D(GaussianModel):
    ...
    def __init__(
        ...
        fg_motion = fg_motion.replace("3dgs-", "").replace("gs-", "")
        ...
        self.background_feat = torch.tensor([0 for i in range(3+16+2+3+1)], dtype=torch.float32, device="cuda")
    ...
    def init_proxy(self, init_scale=0.2): # refactored!
        ...
        if not self.opts["gs_init_mesh"] == "":
            ...
            if not self.opts["gs_init_ply"] == "":
                ...
                xyz = self._xyz.detach().to("cpu").numpy()
            else:
        ...
        if not hasattr(self, "_regist_feat"):
            self._regist_feat = torch.zeros(self._xyz.shape[0], 16, device="cuda")
    ...
    def flip_pair(tensor):
        ...
        if torch.is_tensor(tensor):
            ...
            return tuple([DeformableGaussian3D.flip_pair(t) for t in tensor])
        elif isinstance(tensor, dict):
            return {k: DeformableGaussian3D.flip_pair(v) for k, v in tensor.items()}
...
#--------------------------from deformable.py-----------------
    ...
    def query_field(self, samples_dict, flow_thresh=None):
        ...
        if "gauss_density" in gauss_field.keys():
            gauss_density = gauss_field["gauss_density"]
        else:
            gauss_density = torch.zeros_like(self._xyz[..., :1])
        ...
        TEST_CAM = False
        if False : # no warp no field2cam
            xyz_cam_t, rotation_t = (xyz_reshape, rotation_reshape)
            qt_forward = None
        elif False: # warp + field2cam
        ...
        elif "no_warp" in samples_dict.keys(): # nowarp + field2cam
            q, t = samples_dict["field2cam"]
            q = q[:, None].repeat(1, xyz_reshape.shape[1], 1)
            t = t[:, None].repeat(1, xyz_reshape.shape[1], 1)
            xyz_cam_t, rotation_t = self.apply_qt_to_gaussian(xyz_reshape, rotation_reshape, q, t, frame_id.shape[0])
            qt_forward = None
        else: # warp + no field2cam
            xyz_cam_t, rotation_t, qt_forward = self.forward_warp(xyz_reshape, rotation_reshape, frame_id, inst_id, samples_dict, cache_aux_dict=True, skip_f2c=TEST_CAM)
        if rotation_t.ndim == 3:
            rotation_t = rotation_t[:, :, None, :]
        # compute normal
        ...
        if TEST_CAM:
            ...
            R = R.T
            # FoVx = 2 * torch.arctan(0.5 * Kmat[0, 2] / Kmat[0, 0]) * 180 / torch.pi
            # FoVy = 2 * torch.arctan(0.5 * Kmat[1, 2] / Kmat[1, 1]) * 180 / torch.pi
            FoVx = 2*torch.arctan((samples_dict['Kinv'][0,0,0] * samples_dict["W"])/2)
            FoVy = 2*torch.arctan((samples_dict['Kinv'][0,1,1] * samples_dict["H"])/2)
            idx = 0#len(self.ref_cams)
            Rc2w = R[0].T
            Tc2w = -Rc2w @ (T.T)
            assert R.shape == (3, 3)
            test_cam = Camera(idx, R, T, FoVx, FoVy, torch.zeros(1,samples_dict["H"], samples_dict["W"]), None, str(idx), idx)
            # test_cam = Camera(idx, R[0], T[0]*0, FoVx, FoVy, torch.zeros(1,samples_dict["H"], samples_dict["W"]), None, str(idx), idx, trans=T[0])
            # test_cam = Camera(idx, Rc2w, Tc2w.T, FoVx, FoVy, torch.zeros(1,samples_dict["H"], samples_dict["W"]), None, str(idx), idx)
            gs_camera = [test_cam]
        rendered = {}
        # import ipdb;ipdb.set_trace()
        ...
        for i in range(batch_size):
            ...
            rendered_i = self.render_view(gs_camera[i], override_xyz=xyz_cam_t[i,:,0], 
                                            override_rotation=rotation_t[i,:,0],
        ...
        feat_dict["depth"] = rendered['depth']
    ...
    def forward_warp(self, xyz, rotation, frame_id, inst_id, samples_dict={}, cache_aux_dict=False, skip_f2c=False):
